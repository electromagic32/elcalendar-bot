...
- Имя автора и последнего редактора на карточке события
- Несколько напоминаний на одно событие с повтором и настраиваемым интервалом
- Уведомление в группу при удалении события
- Удалить событие может только создатель или администратор группы
- Создание нового события на основе прошедшего (кнопка 🔁)
...
HOME_GROUP_IDS=-1001234567890  # chat_id групп через запятую (пусто = без ограничений)
```
...
- Описание события
- Карточка показывает автора (👤) и редактора (✏️) если есть
- Повтор прошедшего события: кнопка 🔁 открывает форму с теми же данными и датой +7 дней

## Ограничение доступа

Бот поддерживает whitelist по членству в группе. Только участники указанных групп могут использовать бота и Mini App.

```env
HOME_GROUP_IDS=-1001234567890,-1009876543210
```

- Пусто (по умолчанию) — доступ для всех
- Поддерживается несколько групп через запятую
- Участники со статусом `member`, `administrator`, `creator` — получают доступ
...
**Авто-очистка событий в dev:** установи `EVENT_TTL_MINUTES=10` в `.env.dev` — события старше 10 минут будут удаляться автоматически. В production переменная не задаётся (по умолчанию 0 = выключено).
